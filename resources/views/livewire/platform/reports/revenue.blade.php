...
$byHospital = computed(fn () => app(ReportService::class)->revenueEstimateByHospital($this->date_from ?: null, $this->date_to ?: null));

$total = computed(fn () => $this->byHospital->sum('total'));

$exportCsv = function () {
    abort_unless((bool) Auth::user()?->is_platform_admin, 403);

    $rows = app(ReportService::class)->revenueEstimateByHospital($this->date_from ?: null, $this->date_to ?: null);

    return response()->streamDownload(function () use ($rows) {
        $handle = fopen('php://output', 'w');
        fputcsv($handle, ['hospital', 'revenue']);
        ...
        fclose($handle);
    }, 'revenue-by-hospital-'.now()->format('Y-m-d').'.csv');
};
...
<div class="flex h-full w-full flex-1 flex-col gap-6 rounded-xl">
    <div class="flex items-center justify-between">
        <flux:heading size="xl">{{ __('Revenue estimado por hospital') }}</flux:heading>
            ...
            <table class="min-w-full divide-y divide-zinc-200 dark:divide-zinc-700">
                <thead class="bg-zinc-50 dark:bg-zinc-800/50">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-right text-xs font-semibold text-zinc-500 tracking-wider">{{ __('Revenue') }}</th>
                    </tr>
                </thead>
                <tbody class="bg-white dark:bg-zinc-900 divide-y divide-zinc-200 dark:divide-zinc-700">
                    @forelse($this->byHospital as $row)
                        <tr>
                            ...
                            <td class="px-4 py-3 text-sm text-right font-medium">Q{{ number_format($row['total'], 2) }}</td>
                        </tr>
                    @empty
                        <tr>
